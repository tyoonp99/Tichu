...
from gym_agents import BehaviorCloningAgent  # noqa: E402
...
from nn_training.behavior_cloning import CandidatePolicy  # noqa: E402
from nn_training.imitation_features import FeatureEncoder, FeatureSchema  # noqa: E402


def make_checkpoint(path, state):
...
def test_behavior_cloning_agent_loads_checkpoint_and_returns_legal_action(
    tmp_path, state_factory
):
    state = state_factory(
        [
    ...
    make_checkpoint(checkpoint, state)

    agent = BehaviorCloningAgent(checkpoint, device="cpu")
...
def test_benchmark_factory_creates_behavior_cloning_agent(tmp_path, state_factory):
    state = state_factory(
        [
            {Card.TWO_JADE, Card.A_JADE},
            {Card.THREE_JADE},
    ...
    agent = make_agent(
        "behavior-cloning", bc_checkpoint=checkpoint, bc_device="cpu"
    )
